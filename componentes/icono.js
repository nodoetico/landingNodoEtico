// Componente de iconos SVG inline
// Iconos de Lucide (https://lucide.dev/) - Licencia MIT
...
export function crearIcono(nombre, tamano = 24, claseAdicional = '') {
  if (!iconosDisponibles[nombre]) {
    console.warn(`Icono no encontrado: ${nombre}`);
    return '';
  }

  const claseBase = claseAdicional ? `icono ${claseAdicional}` : 'icono';
  ...
  return `
    <svg
      ...
      aria-hidden="true"
    >
      ${iconosDisponibles[nombre]}
    </svg>
  `;
}

export function renderizarIcono(nombre, tamano = 24, claseAdicional = '') {
  const elemento = document.createElement('span');
  elemento.innerHTML = crearIcono(nombre, tamano, claseAdicional);
  return elemento.firstElementChild;
}

export default crearIcono;